...
class LUT:
        ...
        for x in xrange(newCubeSize):
            for y in xrange(newCubeSize):
                for z in xrange(newCubeSize):
                    newLattice[x, y, z] = self.ColorAtInterpolatedLatticePoint(
    ...
    def _ResizeAndAddToData(self, newCubeSize, data, progress=False):
        ...
        bar = Bar("Building search tree", max=maxVal,
                  suffix='%(percent)d%% - %(eta)ds remain')
        try:
            for x in xrange(newCubeSize):
                if progress:
                    bar.next()
    ...
    def Reverse(self, progress=False):
        ...
        bar = Bar("Searching for matches", max=maxVal,
                  suffix='%(percent)d%% - %(eta)ds remain')
        try:
        ...
        for x in xrange(cubeSize):
            for y in xrange(cubeSize):
                for z in xrange(cubeSize):
                    newLattice[x, y, z] = self.ColorAtLatticePoint(
                        x, y, z).ClampColor(min, max)
    ...
    def _LatticeTo3DLString(self, bitdepth):
        ...
        for currentCubeIndex in range(0, cubeSize**3):
            ...
            greenIndex = ((currentCubeIndex %
                           (cubeSize*cubeSize)) / (cubeSize))
            blueIndex = currentCubeIndex % cubeSize

            latticePointColor = self.lattice[redIndex,
                                             greenIndex, blueIndex].Clamped01()

            string += latticePointColor.FormattedAsInteger(
                2**bitdepth-1) + "\n"
    ...
    def ToLustre3DLFile(self, fileOutPath, bitdepth=12):
        ...
        if int(inputDepth) != inputDepth:
            raise NameError(
                "Invalid cube size for 3DL. Cube size must be 2^x + 1")

        lutFile = open(fileOutPath, 'w')

        lutFile.write("3DMESH\n")
        lutFile.write("Mesh " + str(int(inputDepth)) +
                      " " + str(bitdepth) + "\n")
        lutFile.write(' '.join([str(int(x))
                                for x in Indices(cubeSize, 2**10 - 1)]) + "\n")
    ...
    def ToNuke3DLFile(self, fileOutPath, bitdepth=16):
        ...
        lutFile.write(
            ' '.join([str(int(x)) for x in Indices(cubeSize, 2**bitdepth - 1)]) + "\n")
    ...
    def ToCubeFile(self, cubeFileOutPath):
        ...
        for currentCubeIndex in range(0, cubeSize**3):
            redIndex = int(currentCubeIndex % cubeSize)
            greenIndex = int((currentCubeIndex %
                              (cubeSize*cubeSize)) / (cubeSize))
    ...
    def ToFSIDatFile(self, datFileOutPath):
        ...
        for currentCubeIndex in range(0, cubeSize**3):
            ...
            latticePointColor = lut.lattice[redIndex,
                                            greenIndex, blueIndex].Clamped01()

            rgb_packed = (Remap01ToInt(latticePointColor.r, 1008) | Remap01ToInt(
                latticePointColor.g, 1008) << 10 | Remap01ToInt(latticePointColor.g, 1008) << 20)
        ...
        header_bytes.append(struct.pack("<L", 0x01000002)
                            )  # spec version number?
        # monitor ID (real ID not required if dit.dat file)
        header_bytes.append(bytearray("None".ljust(16)))
        header_bytes.append(bytearray("V1.0".ljust(16)))  # lut version number
        header_bytes.append(struct.pack("<L", lut_checksum))
        # number of bytes in LUT (always the same)
        header_bytes.append(struct.pack("<L", 1048576))
    ...
    def ColorAtLatticePoint(self, redPoint, greenPoint, bluePoint):
        ...
        if redPoint > cubeSize-1 or greenPoint > cubeSize-1 or bluePoint > cubeSize-1:
            raise NameError("Point Out of Bounds: (" + str(redPoint) +
                            ", " + str(greenPoint) + ", " + str(bluePoint) + ")")
    ...
    def ColorAtInterpolatedLatticePoint(self, redPoint, greenPoint, bluePoint):
        ...
        C000 = self.ColorAtLatticePoint(
            lowerRedPoint, lowerGreenPoint, lowerBluePoint)
        C010 = self.ColorAtLatticePoint(
            lowerRedPoint, lowerGreenPoint, upperBluePoint)
        C100 = self.ColorAtLatticePoint(
            upperRedPoint, lowerGreenPoint, lowerBluePoint)
        C001 = self.ColorAtLatticePoint(
            lowerRedPoint, upperGreenPoint, lowerBluePoint)
        C110 = self.ColorAtLatticePoint(
            upperRedPoint, lowerGreenPoint, upperBluePoint)
        C111 = self.ColorAtLatticePoint(
            upperRedPoint, upperGreenPoint, upperBluePoint)
        C101 = self.ColorAtLatticePoint(
            upperRedPoint, upperGreenPoint, lowerBluePoint)
        C011 = self.ColorAtLatticePoint(
            lowerRedPoint, upperGreenPoint, upperBluePoint)
        ...
        for r in xrange(cubeSize):
            for g in xrange(cubeSize):
                for b in xrange(cubeSize):
                    identityLattice[r, g, b] = Color(
                        indices01[r], indices01[g], indices01[b])
    ...
    def FromLustre3DLFile(lutFilePath):
        ...
        for line in lutFileLines[meshLineIndex+1:]:
            if len(line) > 0 and len(line.split()) == 3 and "#" not in line:
                ...
                lattice[redIndex, greenIndex, blueIndex] = Color.FromRGBInteger(
                    redValue, greenValue, blueValue, bitdepth=outputDepth)
                currentCubeIndex += 1
    ...
    def FromNuke3DLFile(lutFilePath):
        ...
        outputDepth = int(
            math.log(int(lutFileLines[meshLineIndex].split()[-1])+1, 2))
    ...
    def FromCubeFile(cubeFilePath):
        ...
        for line in cubeFileLines[cubeSizeLineIndex+1:]:
            if len(line) > 0 and len(line.split()) == 3 and "#" not in line:
                ...
                lattice[redIndex, greenIndex, blueIndex] = Color(
    ...
    def FromFSIDatFile(datFilePath):
        ...
        for currentCubeIndex in xrange(len(lutBytes)/4):
            rgb_packed = np.uint32(struct.unpack(
                "<L", lutBytes[currentCubeIndex*4:(currentCubeIndex*4)+4])[0])
